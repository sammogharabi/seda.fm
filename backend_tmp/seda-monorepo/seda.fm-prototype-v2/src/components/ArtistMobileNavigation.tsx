...
import { BarChart3, Radio, Store, Plus, Home, Compass } from 'lucide-react';
import { motion } from 'motion/react';

interface ArtistMobileNavigationProps {
  currentView: string;
  onViewChange: (view: string) => void;
  onCreateClick?: () => void;
}

export function ArtistMobileNavigation({ currentView, onViewChange, onCreateClick }: ArtistMobileNavigationProps) {
  ...
  const rightNavItems = [
    { id: 'artist-store', icon: Store, label: 'Store' },
    { id: 'artist-dashboard', icon: BarChart3, label: 'Dashboard' }
  ];
  ...
  return (
    <nav className="artist-mobile-navigation fixed bottom-0 left-0 right-0 z-50 border-t border-foreground/10 bg-background/95 backdrop-blur-sm pb-safe">
