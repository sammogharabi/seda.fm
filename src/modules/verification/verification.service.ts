...
export class VerificationService {
  ...
  async requestVerification(userId: string) {
    ...
    const existingRequest = await this.prisma.verificationRequest.findFirst({
      where: {
        userId,
        status: {
          in: [VerificationStatus.PENDING, VerificationStatus.CRAWLING, VerificationStatus.AWAITING_ADMIN],
        },
      },
    });
  ...
  async submitVerification(userId: string, dto: SubmitVerificationDto) {
    ...
    this.crawlerService.verifyClaim(verificationRequest.id, dto.targetUrl, dto.claimCode).catch((error) => {
  ...
  private validateUrl(url: string) {
    try {
      ...
      const isPersonalWebsite = !allowedDomains.some((domain) => parsedUrl.hostname.includes(domain));
