...
export function ArtistDashboard({ 
  ...
  // Generate comprehensive analytics data based on selected timeframe
  const analytics = useMemo(() => {
    const multipliers = {
      ...
      month: 4.2,
      year: 52,
      custom: analyticsDateRange.from && analyticsDateRange.to 
        ? Math.max(1, Math.ceil((analyticsDateRange.to.getTime() - analyticsDateRange.from.getTime()) / (1000 * 60 * 60 * 24 * 7)))
        : 1
    };

    const activeTimeframe = (analyticsDateRange.from && analyticsDateRange.to) ? 'custom' : analyticsTimeframe;
    const multiplier = multipliers[activeTimeframe as keyof typeof multipliers];

    return {
      overview: {
        totalPlays: Math.round(45620 * multiplier),
        totalFans: Math.round(2847 * (multiplier * 0.3 + 0.7)),
        totalTracks: 47,
  ...
  const MOCK_PROFILE_ANALYTICS = useMemo(() => {
    ...
    return {
      ...
      viewGrowth: 15.3, // percentage growth stays constant
      trafficSources: [
      ...
      topViewers: [
        { type: 'Fans', count: Math.round(2134 * multiplier), percentage: 74.9 },
        { type: 'Artists', count: Math.round(713 * multiplier), percentage: 25.1 }
      ]
    };
  ...
  const storeStats = {
    totalStoreRevenue: 847.50,
    thisMonthStore: 234.75,
    storeGrowth: 24,
    totalItems: 12,
    breakdown: {
      tracks: 420.25,
    ...
    topSellingItems: [
      ...
      { name: 'Underground Sessions Vol. 3', sales: 7, revenue: 105.00, type: 'concert' }
    ],
    recentSales: [
      { item: 'Digital Dreams', amount: 3.00, date: '2 hours ago', type: 'track' },
      { item: 'Underground Vibes T-Shirt', amount: 25.00, date: '1 day ago', type: 'merch' },
      { item: 'Underground Sessions Vol. 3', amount: 15.00, date: '2 days ago', type: 'concert' }
    ]
  };
