...
export function LoginPage({ onLogin, onBack }: LoginPageProps) {
  const [authMode, setAuthMode] = useState('login'); // login, signup, username, genres, rooms, welcome, artist-room, artist-track, artist-invite
  ...
  const handleAuth = async () => {
    ...
    try {
      ...
      if (authMode === 'login') {
        ...
        // For fan signup, go to streamlined username selection
        if (userType === 'fan') {
          clearErrors();
          toast.success('Account created successfully!');
          setAuthMode('username');
        } else {
          // For artists, go to artist onboarding flow
          clearErrors();
          toast.success('Account created! Let\'s set up your artist profile.');
          setAuthMode('username'); // Artists also start with username selection
        }
      }
    } catch (error) {
  ...
  const handleAuthError = (error) => {
    ...
    // Log for debugging but don't spam console in demo mode
    console.log('Demo authentication flow - handling simulated error:', errorMessage);
  ...
  const completeFanOnboarding = () => {
    const newUser = {
      ...
      isArtist: false,
      website: '',
      joinedRooms: formData.selectedRooms
    };
  ...
  const completeArtistOnboarding = () => {
    const newUser = {
      id: Date.now(),
      username: formData.username,
      displayName: formData.username,
      email: formData.email,
      verified: false, // Verification is now optional and can be requested later
      verificationStatus: 'not-requested',
      points: 0,
      accentColor: 'coral',
      ...
      location: '',
      joinedDate: new Date(),
      genres: [], // Artists can set genres later
      connectedServices: ['Email'],
      isArtist: true,
      website: '',
      artistRoom: formData.artistRoom,
      pinnedTrack: formData.pinnedTrack,
      invitedFans: formData.inviteEmails
    };
    ...
    <div className="min-h-screen bg-background flex">
      ...
      <div className="hidden lg:flex lg:w-1/2 bg-gradient-to-br from-background via-card to-background relative overflow-hidden">
        ...
        <div className="absolute inset-0 opacity-5">
          <div className="absolute top-20 left-20 w-32 h-32 border-2 border-accent-coral rotate-45"></div>
          <div className="absolute top-40 right-32 w-24 h-24 border-2 border-accent-mint rotate-12"></div>
          <div className="absolute bottom-32 left-40 w-28 h-28 border-2 border-accent-blue -rotate-12"></div>
          <div className="absolute bottom-20 right-20 w-20 h-20 border-2 border-accent-yellow rotate-45"></div>
        </div>
          ...
          >
            ...
            <div className="mb-12">
              <div className="flex items-center gap-4 mb-6">
                <div className="w-16 h-16 bg-accent-coral text-background flex items-center justify-center font-black text-2xl border-2 border-accent-coral">
                  S
                </div>
                <div>
              ...
              <div className="bg-accent-coral text-background p-3 inline-block font-mono text-sm border-2 border-accent-coral mb-6">
            ...
            <div className="space-y-6 mb-12">
              <div className="flex items-start gap-4">
                <div className="w-12 h-12 bg-accent-mint/20 border-2 border-accent-mint flex items-center justify-center">
                  <Mic className="w-6 h-6 text-accent-mint" />
                </div>
                <div>
              ...
              <div className="flex items-start gap-4">
                <div className="w-12 h-12 bg-accent-yellow/20 border-2 border-accent-yellow flex items-center justify-center">
                ...
                {authMode === 'signup' && (
                  <div className="grid grid-cols-2 gap-4 mb-6">
                    ...
                    >
                      ...
                      <div className="font-black text-xs mb-1">FAN PASS</div>
                    ...
                    >
                      ...
                      <div className="font-black text-xs mb-1">ARTIST PASS</div>
                    ...
                    {authMode === 'signup' && (
                      <div>
                        ...
                        <Input
                          ...
                          className={`border-2 focus:border-accent-coral font-mono h-12 mt-2 ${
                            errors.username ? 'border-destructive' : 'border-foreground/20'
                          }`}
                          disabled={isLoading}
                        />
                        {errors.username && (
            ...
            {authMode === 'artist-track' && (
              ...
              >
                ...
                <div className="text-center">
                  ...
                  <p className="text-muted-foreground text-sm mb-6">
                    Showcase your music at the top of your artist room
                  </p>
                </div>
                ...
                <div className="space-y-3">
                  ...
                  <Input
                    ...
                    onChange={(e) => {
                      setFormData(prev => ({ ...prev, trackUrl: e.target.value }));
                      // Clear errors when user starts typing
                      if (errors.trackUrl) {
                        setErrors(prev => ({ ...prev, trackUrl: '' }));
                      }
                      setApiError('');
                    }}
                    placeholder="https://open.spotify.com/track/... or YouTube, SoundCloud, Apple Music link"
                    className={`h-12 text-base ${
                      errors.trackUrl ? 'border-destructive' : ''
                    }`}
                    disabled={isLoading}
                  />
                  {errors.trackUrl ? (
                    <p className="text-destructive text-xs">{errors.trackUrl}</p>
                  ) : (
                    <p className="text-xs text-muted-foreground">
                      This track will be pinned at the top of your artist room for fans to discover
                    </p>
                  )}
                </div>

                {/* Pinned Track Preview */}
                {formData.pinnedTrack && (
                  <Card className="border border-accent-mint bg-accent-mint/10">
                    <CardContent className="p-4">
                      <h3 className="font-semibold text-sm mb-2">Pinned Track Preview</h3>
                      <div className="flex items-center gap-3">
                        <div className="w-12 h-12 bg-accent-mint/20 border border-accent-mint rounded flex items-center justify-center">
                          <Music className="w-5 h-5 text-accent-mint" />
                        </div>
                        <div>
                          <p className="font-medium text-sm">{formData.pinnedTrack.title}</p>
                          <p className="text-xs text-muted-foreground">
                            by {formData.pinnedTrack.artist} • {formData.pinnedTrack.platform}
                          </p>
                        </div>
                      </div>
                    </CardContent>
                ...
                <div className="space-y-3">
                  <Button
                    onClick={handleTrackSubmit}
                    disabled={!formData.trackUrl.trim() || isLoading}
                    className="w-full bg-accent-mint text-background hover:bg-accent-mint/90 h-12 font-semibold disabled:opacity-50"
                  >
                    {isLoading ? (
                      <div className="flex items-center gap-2">
                ...
                <Button
                  variant="ghost"
                  onClick={() => setAuthMode('artist-room')}
                  className="w-full text-sm"
                >
                  ← Back
                </Button>
            ...
            {authMode === 'artist-invite' && (
              ...
              >
                ...
                <div className="text-center">
                  ...
                  <p className="text-muted-foreground text-sm mb-6">
                    Invite fans to join your room and discover your music (optional)
                  </p>
                </div>
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex gap-2">
                    <Input
                      id="inviteEmail"
                      type="email"
                      placeholder="Enter fan email address..."
                      className="h-12 text-base flex-1"
                      onKeyPress={(e) => {
                        if (e.key === 'Enter') {
                          const email = e.target.value.trim();
                          if (email) {
                            handleAddInviteEmail(email);
                            e.target.value = '';
                          }
                        }
                      }}
                  ...
                  <p className="text-xs text-muted-foreground">
                    Press Enter or click Add to include each email address
                  </p>
                </div>
                ...
                {formData.inviteEmails.length > 0 && (
                  <div className="space-y-2">
                    <h3 className="text-sm font-medium">Invite List ({formData.inviteEmails.length})</h3>
                    <div className="max-h-32 overflow-y-auto space-y-1">
                      {formData.inviteEmails.map((email, index) => (
                        <div
                          key={index}
                          className="flex items-center justify-between bg-accent-blue/10 border border-accent-blue/20 p-2 rounded"
                        >
                ...
                <Button
                  ...
                  className="w-full bg-accent-yellow text-background hover:bg-accent-yellow/90 h-12 font-semibold"
                >
                  {formData.inviteEmails.length > 0 
                    ? `Send ${formData.inviteEmails.length} Invite${formData.inviteEmails.length > 1 ? 's' : ''}` 
                    : 'Skip for Now'
                  }
                </Button>
                ...
                {formData.inviteEmails.length > 0 && (
                  <Button
                    onClick={() => {
                      setFormData(prev => ({ ...prev, inviteEmails: [] }));
                      handleInviteSubmit();
                    }}
                    variant="ghost"
                    className="w-full text-sm"
                  >
                    Skip and invite fans later
                  </Button>
                )}
                ...
                <Button
                  variant="ghost"
                  onClick={() => setAuthMode('artist-track')}
                  className="w-full text-sm"
                >
                  ← Back
                </Button>
