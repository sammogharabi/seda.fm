      ...
      >
        ...
        <div className="relative flex items-center justify-between">
          <div className="flex items-center gap-4">
            <motion.div 
              className={`${isMobile ? 'w-8 h-8' : 'w-12 h-12'} bg-ring/10 rounded-xl flex items-center justify-center relative`}
              animate={{ 
                boxShadow: isPlaying ? 
            ...
            >
              ...
              >
                <Disc className={`${isMobile ? 'w-4 h-4' : 'w-6 h-6'} text-ring`} />
              </motion.div>
              {isPlaying && (
                <motion.div
            ...
            <div className="flex-1 min-w-0">
              <div className={`flex items-center ${isMobile ? 'gap-2' : 'gap-3'} ${isMobile ? 'flex-wrap' : ''}`}>
                <h2 className={`${isMobile ? 'text-lg' : 'text-xl'} font-medium ${isMobile ? 'truncate' : ''}`}>
                  {isMobile ? 'Session Live' : `Session - ${room}`}
                </h2>
              ...
              <div className={`flex items-center ${isMobile ? 'gap-2 mt-1' : 'gap-4 mt-1'} ${isMobile ? 'flex-wrap' : ''}`}>
                ...
                <div className="flex items-center gap-1 text-xs text-muted-foreground">
                  ...
                  <span>{sessionStats.totalTracks}</span>
                </div>
                {sessionConfig && (
                  ...
                  <div className={`flex ${isMobile ? 'gap-3' : 'gap-6'}`}>
                    ...
                    <div className="flex-1">
                      ...
                      <div className="flex items-center gap-3 mb-4">
                        <div className="w-7 h-7 bg-gradient-to-br from-accent-coral to-accent-mint border-2 border-foreground shadow-lg flex items-center justify-center font-black text-background text-sm">
                    ...
                    <div className={`flex ${isMobile ? 'flex-row gap-2' : 'flex-col gap-3'}`}>
                      <motion.div whileTap={{ scale: 0.95 }}>
                        <Button 
                          ...
                          className={`${isMobile ? 'h-10 w-10' : 'h-14 w-14'} shadow-xl hover:shadow-2xl transition-all duration-200 bg-gradient-to-br from-primary to-primary/80`}
                        >
                          <motion.div
                            animate={isPlaying ? { scale: [1, 1.1, 1] } : {}}
                            transition={{ duration: 0.3 }}
                          >
                            {isPlaying ? <Pause className={`${isMobile ? 'w-5 h-5' : 'w-7 h-7'}`} /> : <Play className={`${isMobile ? 'w-5 h-5' : 'w-7 h-7'}`} />}
                          </motion.div>
                        </Button>
                      </motion.div>
                      {isHost && (
                      ...
                      <Button 
                        size="sm"
                        variant="ghost" 
                        className={`${isMobile ? 'h-10 w-10' : 'h-8 w-14'}`}
                      >
                      ...
                      <div className="space-y-3">
                        {listeners.map((listener) => (
                          <div key={listener.username} className="flex items-center gap-3">
                            <div className="w-6 h-6 bg-gradient-to-br from-accent-yellow to-accent-coral border border-foreground flex items-center justify-center font-black text-background text-xs">
                      ...
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="flex items-center gap-2 mt-1">
                            <div className="w-4 h-4 bg-gradient-to-br from-accent-blue to-accent-mint border border-foreground flex items-center justify-center font-black text-background text-xs">
              ...
              <CardContent>
                <ScrollArea className="h-64">
                  <div className="space-y-3">
                    {listeners.map((listener) => (
                      <div key={listener.username} className="flex items-center gap-3">
                        <div className="w-7 h-7 bg-gradient-to-br from-accent-mint to-accent-blue border-2 border-foreground flex items-center justify-center font-black text-background text-sm">
                          {listener.username[0].toUpperCase()}
                        </div>
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-1">
