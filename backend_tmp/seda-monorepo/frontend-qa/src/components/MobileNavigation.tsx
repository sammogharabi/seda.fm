...
import { Home, Compass, Music, User, Users, Plus, Search, Bookmark, MessageSquare } from 'lucide-react';
...
export function MobileNavigation({ currentView, onViewChange, isDJMode, onCreatePost, onSearch }: MobileNavigationProps) {
  ...
  const navItems = [
    ...
    { id: 'playlists', icon: Bookmark, label: 'Playlists' },
    { id: 'profile', icon: User, label: 'Profile' }
  ];
  
  return (
    <motion.nav 
      className="fixed bottom-0 left-0 right-0 z-40 border-t border-border bg-background mobile-safe-bottom"
      initial={{ y: 100 }}
      animate={{ y: 0 }}
